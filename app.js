...
function isProbablyStoragePath(u){
  ...
  // common Firebase Storage relative paths
...
function cloudinaryUrl(u){
  ...
  let safe = s.replace(/^\/+/, "");
  // If a bare public_id is provided without folder, assume default folder
  const FOLDER = "Joodkids";
  if (FOLDER && !safe.includes("/") && !safe.startsWith(FOLDER + "/")) {
    safe = FOLDER + "/" + safe;
  }
  // Use automatic format/quality for better performance
  return `https://res.cloudinary.com/dthtzvypx/image/upload/f_auto,q_auto/${safe}`;
}
async function resolveImageUrl(raw){
  ...
  // Firebase Storage (gs://... or relative path like products/xxx.jpg)
  if (isProbablyStoragePath(u)) {
    try {
      const r = storageRef(storage, u);
