...
function normalizeUrl(u){
  ...
  if (m1) u = `https://drive.google.com/uc?export=view&id=${m1[1]}`;
  else if (m2) u = `https://drive.google.com/uc?export=view&id=${m2[1]}`;
  // Force https
...
function firstImage(p){
  ...
  if(arr.length) return normalizeUrl(arr[0]);
  if(p?.image) return normalizeUrl(p.image);
  if(p?.imageUrl) return normalizeUrl(p.imageUrl);
  if(p?.img) return normalizeUrl(p.img);
  return "logo.png";
}
...
function renderGrid() {
  ...
  for (const p of state.filtered) {
    ...
    card.innerHTML = `
      <div class="card__img">
        <img src="${img}" alt="${escapeHtml(p.name||"")}" loading="lazy" onerror="this.onerror=null;this.src=\'logo.png\';"/>
...
function renderCart() {
  ...
  if (!state.cart.length) {
    ...
    for (const it of state.cart) {
      ...
      row.innerHTML = `
        <img src="${it.image||"logo.png"}" alt="" onerror="this.onerror=null;this.src=\'logo.png\';"/>
        <div class="ciBody">
...
function renderMenuCategories() {
  ...
  all.className = "chip" + (state.filterCategory ? "" : " is-active");
  all.type = "button";
  ...
  for (const k of state.categories) {
    ...
    b.className = "chip" + (String(state.filterCategory) === String(k) ? " is-active" : "");
    b.type = "button";
    b.textContent = "تصنيف " + k;
    b.addEventListener("click", ()=>{
      state.filterCategory = String(k);
      renderMenuCategories();
      applyFilters();
...
function renderMenuSeasons() {
  const wrap = $("menuSeasons");
  wrap.innerHTML = "";
  const seasons = ["", "صيفي", "خريفي", "شتوي"];
  const labels = {"": "الكل", "صيفي":"صيفي", "خريفي":"خريفي", "شتوي":"شتوي"};
  for (const s of seasons) {
    const b = document.createElement("button");
    b.className = "chip" + (state.filterSeason === s ? " is-active" : "");
    b.type = "button";
