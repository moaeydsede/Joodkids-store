...
function openInstall(){
  document.getElementById("installBackdrop").classList.remove("hidden");
  document.getElementById("installCard").classList.remove("hidden");
}
function closeInstall(){
  document.getElementById("installBackdrop").classList.add("hidden");
...
let PRODUCTS = [];
let ACTIVE = null;
let ACTIVE_SEASON = null; // 'summer'|'autumn'|'winter'
let ACTIVE_GROUP = null;  // model group prefix
...
function seasonLabel(v){
  if (v === "summer") return "صيفي";
  if (v === "autumn") return "خريفي";
  if (v === "winter") return "شتوي";
  return "غير محدد";
}
function modelGroupKey(model){
  ...
  if (n < 1000) return String(n)[0];       // أول رقم
  return String(n).slice(0,2);             // أول رقمين
}
...
function applyFilters(){
  ...
  if (ACTIVE_SEASON){
    list = list.filter(p => (p.season || null) === ACTIVE_SEASON);
  }
  if (ACTIVE_GROUP){
    list = list.filter(p => modelGroupKey(p.model) === ACTIVE_GROUP);
  }
  ...
  if (q){
    list = list.filter(p =>
      ...
      String(parseSizes(p.sizes).join(" ")).toLowerCase().includes(q)
    );
  }

  if (sort === "priceAsc") list.sort((a,b)=>(Number(a.priceWholesale ?? a.price)||0)-(Number(b.priceWholesale ?? b.price)||0));
  else if (sort === "priceDesc") list.sort((a,b)=>(Number(b.priceWholesale ?? b.price)||0)-(Number(a.priceWholesale ?? a.price)||0));
...
function applySettings(s){
  ...
  // Update both header/footer links and menu links (avoid duplicate IDs)
  setLink("igLink", ig);
  setLink("igLinkMenu", ig);
  setLink("fbLink", fb);
  setLink("fbLinkMenu", fb);
  setLink("tgLink", tg);
  setLink("tgLinkMenu", tg);
  setLink("waLink", wa);
  setLink("waLinkMenu", wa);

  const shop = s?.shopMapEmbedUrl || "";
  const factory = s?.factoryMapEmbedUrl || "";
  const shopEls = [$("shopMap"), $("shopMapMenu")].filter(Boolean);
  const facEls  = [$("factoryMap"), $("factoryMapMenu")].filter(Boolean);
...
async function checkout(){
  ...
  try{
    ...
    const ordersNum = (SETTINGS_CACHE?.ordersWhatsappNumber || "").trim();
    if (ordersNum){
      const lines = items.map((it)=>{
        const model = it.model ? `موديل ${it.model}` : "";
        const size = it.size ? `مقاس ${it.size}` : "";
        const qty = `عدد ${it.qty || 1}`;
        return `- ${it.name || ""} ${model} ${size} (${qty})`;
      }).join("
");
...
الإجمالي: ${money(cartTotal(items))}
طريقة الدفع: ${payMethod}
`;
      const url = `https://wa.me/${ordersNum}?text=${encodeURIComponent(msg)}`;
...
function buildSeasonMenu(){
  ...
  seasons.forEach(season => {
    ...
    const groups = {};
    items.forEach(p => {
      const g = modelGroupKey(p.model);
      if (!groups[g]) groups[g] = 0;
      groups[g]++;
    });

    const groupKeys = Object.keys(groups).filter(k=>k!=="—").sort((a,b)=>Number(a)-Number(b));
    ...
    card.innerHTML = `
      <div class="seasonHead">
        <div>
          <div class="seasonTitle">${seasonLabel(season)}</div>
          <div class="seasonSub">${items.length} موديل</div>
        </div>
        <div class="muted">▾</div>
      </div>
      <div class="seasonBody">
        <div class="chips">
          <button class="chip" data-season="${season}" data-group="">الكل <small>(${items.length})</small></button>
          ${groupKeys.map(g=>`<button class="chip" data-season="${season}" data-group="${g}">${g} <small>(${groups[g]})</small></button>`).join("")}
        </div>
      </div>
    `;

    card.querySelector(".seasonHead").onclick = ()=>{
    ...
    card.querySelectorAll(".chip").forEach(btn=>{
      btn.onclick = ()=>{
        ...
        const g = btn.dataset.group || null;

        ACTIVE_SEASON = s || null;
        ACTIVE_GROUP = g || null;
  ...
  // Add "Reset" chip at top (outside cards) via first card's body if needed
}
...
function initAccordions(){
  document.querySelectorAll(".acc .accHead").forEach(btn=>{
...
$("installBackdrop").onclick = closeInstall;
$("closeInstall").onclick = closeInstall;
$("btnInstallLater").onclick = closeInstall;
$("btnInstallNow").onclick = async ()=>{
  try{
    if (!deferredPrompt) return closeInstall();
