...
import {
  ...
  doc, getDoc, setDoc, serverTimestamp, limit, where
    ...
    <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial;direction:rtl;color:#fff;text-align:center">
      <div style="max-width:720px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:18px;padding:22px">
        ...
        <p style="margin:0 0 14px;opacity:.9;line-height:1.7">
          هذا المشروع يستخدم <b>ES Modules</b> واستدعاءات Firebase من الإنترنت، وفتح <b>admin.html</b> من مدير الملفات يمنع تحميل الموديولات.
...
function cloudinaryUrl(u){
  ...
  const allowFirebaseFallback = false; // نستخدم Cloudinary افتراضياً
  if (folder && !safe.includes("/") && !safe.startsWith(folder + "/")) {
    safe = folder + "/" + safe;
  }
  return `https://res.cloudinary.com/${CLOUDINARY_CLOUD}/image/upload/f_auto,q_auto/${safe}`;
}
// Stored value normalization (what we save to Firestore)
function resolveImg(url){
  const u = toHttps(url);
  if (!u) return "";
  // Google Drive shared links -> direct image url (requires public sharing)
  const d = driveDirectUrl(u);
  if (d) return d;
  // keep Firebase Storage references as-is
  if (isProbablyStoragePath(u)) return u;
  // keep full URLs
  if (/^https?:\/\//i.test(u) || u.startsWith("data:") || u.startsWith("blob:")) return u;
  // otherwise treat as Cloudinary public_id
  return u; // keep public_id as-is; app will render it
}
async function previewUrl(raw){
  const u = toHttps(raw);
  ...
  if (isProbablyStoragePath(u)) {
    try {
      return await getDownloadURL(storageRef(storage, u));
    } catch(e){
      return "logo.png";
    }
  }
...
async function saveCompany() {
  const data = {
    ...
    policyText: $("coPolicyText").value.trim(),
    termsText: $("coTermsText").value.trim(),
...
function setupModals() {
  ...
  $("btnCompany").addEventListener("click", async ()=> {
    ...
    $("coPolicyText").value = state.company?.policyText || "";
    $("coTermsText").value = state.company?.termsText || "";
...
function renderGrid() {
  ...
  for (const p of state.filtered) {
    const img = (Array.isArray(p.images) && p.images[0]) ? resolveImg(p.images[0]) : "logo.png";
...
function renderImages() {
  ...
  for (const url of imgs) {
    ...
    previewUrl(url).then(u=>{ if(u) img.src=u; }).catch(()=>{});
...
function addImageUrl() {
  ...
  state.editing.images.push(resolveImg(url));
  $("pImgUrl").value = "";
  renderImages();
}
...
async function uploadImageFile(e) {
  ...
  await loadCompany();
  const preset = (state.company?.cloudinaryPreset || "Joodkids").trim();
  const folder = (state.company?.cloudinaryFolder || "Joodkids").trim() || "Joodkids";
  const allowFirebaseFallback = false; // نستخدم Cloudinary افتراضياً

  // إذا لم يتم ضبط preset (نادر)، يمكن تفعيل fallback
  if (!preset && allowFirebaseFallback) {
    try {
      toast("جاري رفع الصورة إلى Firebase…");
      ...
      console.error(err);
      toast("تعذر رفع الصورة. تأكد من تفعيل Firebase Storage والصلاحيات.");
    }
    return;
  }
...
async function saveProduct() {
  ...
  const data = {
    ...
    desc,
    categoryKey,
    images: Array.isArray(state.editing.images) ? state.editing.images.map(resolveImg) : [],
    updatedAt: serverTimestamp(),
  };
  ...
  // Prevent duplicate product by model (mobile double-tap / accidental re-add)
  const qy = query(collection(db, "products"), where("model","==", model), limit(1));
  const snap = await getDocs(qy);
  if (!snap.empty) {
    const existing = snap.docs[0];
    await updateDoc(doc(db, "products", existing.id), data);
    toast("تم تحديث نفس الموديل (منع تكرار الحفظ)");
  } else {
    ...
    snap.forEach(d => {
      ...
      el.innerHTML = `
        <div class="ciBody">
          ...
          <div class="small">المدينة: <b>${escapeHtml(o.city||"")}</b> • شركة الشحن: <b>${escapeHtml(o.shippingCompany||"")}</b></div>
          <div class="small">الدفع: <b>${escapeHtml(o.paymentMethod||"")}</b></div>
