...
function addImageUrl() {
  ...
  state.editing.images.push(normalizeUrl(url));
  $("pImgUrl").value = "";
  renderImages();
}
...
async function uploadImageFile(e) {
  ...
  await loadCompany();
  let preset = (state.company?.cloudinaryPreset || "").trim();
  if (!preset) preset = "Joodkids";
  ...
  try {
    ...
    if (!state.editing.images) state.editing.images = [];
    state.editing.images.push(normalizeUrl(data.secure_url));
    renderImages();
    toast("تم رفع الصورة");
  } catch (err) {
    console.error(err);
...
// منع تكرار رقم الموديل
const mNorm = model.replace(/\s+/g,"").toLowerCase();
if (mNorm) {
  const dup = state.products.find(p => {
    const pModel = (p.model||"").toString().trim().replace(/\s+/g,"").toLowerCase();
    if (!pModel) return false;
    if (state.editing?.id && p.id === state.editing.id) return false;
    return pModel === mNorm;
  });
  if (dup) {
    toast("⚠️ هذا رقم الموديل موجود مسبقاً");
    return;
  }
}
  ...
  // منع تكرار نفس رقم الموديل
  try {
    const qy = query(collection(db, "products"), where("model", "==", model), limit(1));
    const snap = await getDocs(qy);
    const exists = !snap.empty;
    const sameId = exists ? (snap.docs[0].id === state.editing?.id) : false;
    if (exists && !sameId) {
      toast("هذا الموديل موجود مسبقًا");
      return;
    }
  } catch (e) {
    // ignore - continue
