...
function requireAuthUI(isAuthed){
  ...
  $("btnCompany").classList.toggle("hidden", !isAuthed);
}
...
async function saveProduct(){
  ...
  try{
    ...
    if (ACTIVE_ID){
      ...
      const ref = // Prevent duplicate model
      const q = query(collection(db, "products"), where("model","==", payload.model));
      const snap = await getDocs(q);
      if (!snap.empty) throw new Error("DUP_MODEL");
...
async function loadSettings(){
  ...
  $("sOrdersWA").value = s.ordersWhatsappNumber || "";
...
$("btnCompany").onclick = ()=>{ document.getElementById("settingsPanel")?.scrollIntoView({behavior:"smooth"}); };
